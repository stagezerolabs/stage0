...
const tokenTypes: {
  ...
  functionName: TokenFactoryFunctionName;
}[] = [
  {
    type: 'plain',
    label: 'Plain',
...
const CreateTokenPage: React.FC = () => {
  ...
  const lastSuccessToastHash = useRef<string | null>(null);
  const storedCreatedToken = useRef<string | null>(null);
  ...
  const showSuccessModal = isSuccess && (!hash || dismissedSuccessHash !== hash);
  ...
  useEffect(() => {
    if (!isSuccess || !hash || lastSuccessToastHash.current === hash) return;

    toast.success('Token created successfully.');
    lastSuccessToastHash.current = hash;
  }, [isSuccess, hash]);
  ...
  useEffect(() => {
    ...
    if (storedCreatedToken.current === createdTokenAddress) return;
    ...
    storedCreatedToken.current = createdTokenAddress;
  ...
  const handleSubmit = () => {
    ...
    if (selectedType === 'taxable') {
      ...
      writeContract({
        abi: TokenFactory,
        address: contracts.tokenFactory,
        functionName: tokenTypeConfig.functionName,
        args: [
          {
            name,
            symbol,
    ...
    >
      ...
      {showSuccessModal && (
        <div
          ...
          onClick={() => hash && setDismissedSuccessHash(hash)}
        >
          <motion.div
